...
credit_record as (
    select
        ID as CLIENT_ID,
        {{ convert_status('ID_STATUS') }} as CONVERTED_STATUS
    from `lyrical-country-370008.data_fellowship.fact_credit_record`
),

avg_credit_record as (
    select
        CLIENT_ID,
        AVG(CONVERTED_STATUS) as AVG_STATUS
    from credit_record
    group by CLIENT_ID
),
...
training_dataset as (
    select 
        application_record.*,
        {{ create_prediction('AVG_STATUS') }} as PREDICTION
    from application_record
    left join avg_credit_record
    on application_record.CLIENT_ID = avg_credit_record.CLIENT_ID
)
